...
CREATE FUNCTION [$(schema)].[stripDoubleSpaces](@prmSource varchar(max)) 
RETURNS VARCHAR(max)
AS 
BEGIN
...
CREATE FUNCTION [$(schema)].[fnAlHySpOnly](@string VARCHAR(MAX))
RETURNS VARCHAR(MAX)
BEGIN
...
CREATE FUNCTION [$(schema)].[fnAlphaOnly](@string VARCHAR(MAX))
RETURNS VARCHAR(MAX)
BEGIN
    WHILE PATINDEX('%[^A-Z]%', @string) > 0
	      SET @string = STUFF(@string, PATINDEX('%[^A-Z]%', @string), 1, '')
...
CREATE FUNCTION [$(schema)].[fnNumberOnly](@string VARCHAR(MAX))
RETURNS VARCHAR(MAX)
BEGIN
    WHILE PATINDEX('%[^0-9]%', @string) > 0
	      SET @string = STUFF(@string, PATINDEX('%[^0-9]%', @string), 1, '')
RETURN RIGHT(@string,4)
END
GO
...
CREATE FUNCTION [$(schema)].[fnRemovePrefix2](@Name VARCHAR(MAX))
RETURNS VARCHAR(MAX)
BEGIN
...
CREATE FUNCTION [$(schema)].[fnRemoveSuffix2](@Name VARCHAR(MAX))
RETURNS VARCHAR(MAX)
BEGIN
